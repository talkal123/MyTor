...
const NearbyBusiness = ({data, category,userDetails,loading}) => {
...
const newArr = data.filter((b) => b.address.toLowerCase().includes(userDetails.city))
...
const handleClick = () => {
  ...
  {loading ? (
    <div className='w-full flex flex-col items-center justify-center'>
      <h1 className='font-semibold text-lg mb-3'>Loading {newCategory}s...</h1>
        ...
        {newArr.length === 0 ? (
          <div className='flex flex-col gap-10 items-center justify-center'>
            <p className='text-center mt-5'>
              "No businesses found in your area. Try adjusting your location!"
            </p>
            <img className='w-20 h-20' src={UnHappyPhoto} alt="" />
          </div>
        ) : (
          newArr.slice(0, 3).map((business, index) => (
